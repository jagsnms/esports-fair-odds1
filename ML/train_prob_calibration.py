...
# We default to calibrating on a small set of per-map checkpoints (additive-only; falls back to all rows if missing).
CALIBRATION_MODE = "checkpoints"  # "checkpoints" or "weighted_all"
CHECKPOINT_ROUNDS = [1, 3, 6, 9, 12, 15, 18, 21, 24]

def resolve_path(default_path: Path, argv: List[str], idx: int) -> Path:
    if len(argv) > idx:
        p = Path(argv[idx]).expanduser()
...
def main():
    snap_path = resolve_path(SNAPSHOT_CSV, sys.argv, 1)
    res_path  = resolve_path(RESULTS_CSV, sys.argv, 2)

    if not snap_path.exists():
        raise FileNotFoundError(f"Missing snapshot CSV: {snap_path}")
    if not res_path.exists():
        raise FileNotFoundError(f"Missing results CSV: {res_path} (required for outcome calibration)")
    ...
    report = {
        ...
        "notes": [
            ...
            "To reduce domination by late-round snapshots, this trainer can select per-map checkpoints or weight rows per match." ,
    ...
    for game in sorted(merged["game"].unique().tolist()):
        ...
        # Build a round index for checkpoint selection/weighting (backwards compatible)
        if "round_in_map" in g.columns:
            g["round_in_map"] = pd.to_numeric(g["round_in_map"], errors="coerce")
        elif "total_rounds" in g.columns:
            g["round_in_map"] = pd.to_numeric(g["total_rounds"], errors="coerce")
        elif ("rounds_a" in g.columns) and ("rounds_b" in g.columns):
            g["round_in_map"] = pd.to_numeric(g["rounds_a"], errors="coerce").fillna(0) + pd.to_numeric(g["rounds_b"], errors="coerce").fillna(0)
        elif "snapshot_idx" in g.columns:
            g["round_in_map"] = pd.to_numeric(g["snapshot_idx"], errors="coerce")
        else:
            g["round_in_map"] = np.nan

        g["round_in_map"] = g["round_in_map"].fillna(np.nan)

        # Row weights: equalize matches (each match contributes ~1.0 total weight)
        per_match_n = g.groupby("match_id")["match_id"].transform("count").astype(float)
        w_match = 1.0 / np.maximum(per_match_n, 1.0)

        # Optional checkpoint selection
        g_use = g
        if CALIBRATION_MODE == "checkpoints" and g["round_in_map"].notna().any():
            chk = set(int(x) for x in CHECKPOINT_ROUNDS)
            mask = g["round_in_map"].round().astype("Int64").isin(list(chk))
            # keep all OT rows if explicit OT flags exist
            if "is_ot" in g.columns:
                ot = g["is_ot"]
                if isinstance(ot.iloc[0], str):
                    ot = ot.astype(str).str.strip().str.lower().isin(["1","true","yes","y","t"])
                mask = mask | ot.astype(bool)
            if "half" in g.columns:
                mask = mask | g["half"].astype(str).str.strip().str.lower().isin(["ot","overtime"])
            if mask.any():
                g_use = g[mask].copy()
                w_match = w_match.loc[g_use.index]

        p_raw = _clip01(g_use["p_fair"].to_numpy(dtype=float))
        y = g_use["y"].to_numpy(dtype=int)
        w = w_match.to_numpy(dtype=float)

        xs, ys = pava_isotonic(p_raw, y, w=w)
        ...
        out_json[game] = {
            ...
            "trained_on": {"n_rows": int(len(g_use)), "n_matches": n_matches, "mode": CALIBRATION_MODE},
            "metrics_before": mb,
            "metrics_after": ma,
        }
        ...
        print(f"\n[{game.upper()}] matches={n_matches} rows={len(g_use)} (mode={CALIBRATION_MODE})")
