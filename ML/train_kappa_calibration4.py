...
def main():
    ...
    df["rounds_a"] = df["rounds_a"].astype(int)
    df["rounds_b"] = df["rounds_b"].astype(int)
    df["total_rounds"] = df["rounds_a"] + df["rounds_b"]
    df["bucket"] = df["total_rounds"].apply(bucket_for_total_rounds)
    ...
    report: Dict = {
        ...
        "notes": [
            ...
            "Buckets: early<=8, mid<=18, late<=23, ot>=24 (by total rounds).",
        ],
        "games": {},
        "calibration": {},
        "training": {},
