...
def load_cs2_teams() -> pd.DataFrame:
    df = pd.read_csv("cs2_rankings_merged.csv")  # team, tier, rank, hltv_id, slug
    df["norm_team"] = df["team"].apply(normalize_name)
    return df

@st.cache_data
def load_dota_teams() -> pd.DataFrame:
    df = pd.read_csv("dota2_gosu_rankings.csv")  # team, tier, rank, slug, opendota_id
...
def decide_bet(p_model: float,
    ...
    # Market implied probability for A (vig-aware via American odds)
    p_mkt_a = implied_prob_from_american(odds_a)
    gap_ok = abs(p_decide - p_mkt_a) >= (prob_gap_pp / 100.0)

    # Decision EVs (using p_decide, not raw p_model)
...
# ==========================
def recompute_metrics_from_logs(rows: list[dict]) -> dict:
    """Rebuild Diagnostics counters from logged entries."""
...
def load_persisted_logs() -> list[dict]:
    """Read LOG_PATH if it exists; return list[dict]."""
...
def persist_log_row(entry: dict):
    """Append one row to disk (create file with header if not present)."""
...
with tabs[2]:
    ...
    st.download_button("Download CSV", csv, "fair_odds_logs_snapshot.csv", "text/csv")

    if st.button("Overwrite on-disk log with current in-memory logs"):
        try:
            pd.DataFrame(st.session_state["logs"]).to_csv(LOG_PATH, index=False)
