...
# ==========================
# Calibration Dashboard (reads kappa_calibration_report.json)
# ==========================
...
def _calib_level_progress(n_matches: int):
    for name, target in _CALIB_LEVELS:
        if n_matches < target:
            frac = (n_matches / target) if target else 0.0
            return name, target, frac
    name, target = _CALIB_LEVELS[-1]
    return f"{name} (MAX)", target, 1.0

def render_calibration_dashboard():
    """UI-only viewer. Does NOT train — it reads the last report/calibration written by the trainer."""
    st.header("Kappa Calibration Dashboard")

    if not KAPPA_REPORT_PATH.exists():
        st.warning("No calibration report found yet. Run calibration first (Calibrate button).")
        show_inplay_log_paths()
        return

    try:
        report = json.loads(KAPPA_REPORT_PATH.read_text(encoding="utf-8"))
    except Exception as e:
        st.error(f"Could not read report JSON: {e}")
        st.caption(str(KAPPA_REPORT_PATH))
        return

    calib = load_kappa_calibration()

    st.caption(f"Report updated: {report.get('ran_at', 'unknown')}")
    st.caption(f"Log CSV: {report.get('log_csv', str(INPLAY_LOG_PATH))}")
    ...
    if not games:
        st.info("No game blocks found in report.")
        return

    for game_key, g in games.items():
        st.subheader(game_key.upper())

        n_matches = int((g.get("match_id", {}) or {}).get("unique", 0) or 0)
        rows = int(g.get("rows_usable", 0) or 0)
        buckets = g.get("buckets", {}) or {}
        band_levels = g.get("band_levels", []) or []
        mid_stats = g.get("mid_stats", {}) or {}

        lvl_name, lvl_target, frac = _calib_level_progress(n_matches)

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Matches", n_matches)
        c2.metric("Snapshots", rows)
        c3.metric("Band levels", ", ".join([str(x) for x in band_levels]) if band_levels else "—")
        c4.metric("Mid mean", f"{float(mid_stats.get('mean', 0.0)):.3f}")

        st.write(f"**{lvl_name}** → target **{lvl_target}** matches")
        st.progress(min(max(frac, 0.0), 1.0), text=f"{n_matches}/{lvl_target} ({frac*100:.1f}%)")

        st.write("**Bucket counts:**", {k: int(v) for k, v in buckets.items()})

        # Training results table(s)
        t_game = training.get(game_key.lower(), training.get(game_key.upper(), training.get(game_key, {}))) or {}
        if not t_game:
            st.info("No trained multipliers yet for this game (not enough data).")
            continue

        for band_key, band_block in t_game.items():
            st.markdown(f"### Band level {band_key}")

            # Show the actual multipliers being used by the app (from kappa_calibration.json)
            gcal = calib.get(game_key.lower(), calib.get(game_key.upper(), {})) if isinstance(calib, dict) else {}
            if isinstance(gcal, dict):
                mults = gcal.get(str(band_key), gcal.get(f"{float(band_key):.2f}" if str(band_key).replace('.','',1).isdigit() else str(band_key), {}))
            else:
                mults = {}
            if mults:
                st.write("**Multipliers in use:**", mults)

            rows_out = []
            if isinstance(band_block, dict):
                for scope, d in band_block.items():
                    ...
                    rows_out.append({
                        ...
                        "n": d.get("n"),
                        "target": d.get("target"),
                        "base_cover": d.get("base_cover"),
                        "best_cover": d.get("best_cover"),
                        "base_width": d.get("base_width"),
                        "best_width": d.get("best_width"),
                        "best_mult": d.get("best_mult"),
                    })
            if rows_out:
                st.dataframe(rows_out, use_container_width=True)
            else:
                st.info("No per-scope training rows in report for this band level.")

            if isinstance(band_block, dict) and "global" in band_block:
                gd = band_block["global"]
                try:
                    st.success(
                        f"Global: target {float(gd.get('target', 0)):.2f}, cover {float(gd.get('base_cover', 0)):.3f} → {float(gd.get('best_cover', 0)):.3f}. "
                        f"mult={float(gd.get('best_mult', 1)):.3f} (width {float(gd.get('base_width', 0)):.3f} → {float(gd.get('best_width', 0)):.3f})."
                    )
                except Exception:
                    pass
...
with tabs[3]:
    ...
    # --- Final result logging (needed for ML calibration) ---
    ...
    with rcol1:
        ...
        try:
            ...
            pass
        cs2_winner_sel = st.selectbox("Winner", [team_a_live, team_b_live], index=_cs2_default_winner_idx, key="cs2_inplay_winner_sel")
    with rcol2:
        if st.button("Save result", key="cs2_inplay_save_result"):
            ...
            if not _mid:
                ...
                persist_inplay_result(_mid, "CS2", str(team_a_live), str(team_b_live), str(cs2_winner_sel))
...
# --------------------------
with tabs[5]:
    st.header("Calibration (Kappa Bands)")
    show_inplay_log_paths()
    ...
    with colx:
        if st.button("Run calibration now"):
            ...
            if out:
                with st.expander("Trainer output"):
                    st.code(out)
            # refresh UI (works across Streamlit versions)
            try:
                st.rerun()
            except Exception:
                try:
    ...
    with coly:
        st.caption("This tab reads the latest kappa_calibration_report.json and kappa_calibration.json written by the trainer.")
