...
with colClear:
    ...
    with cB:
        if st.button("Start next half"):
            ...
            # Optional: reset manual chaos widen (keeps things consistent at half)
...
with colClear:
    ...
    with cA:
        if st.button("Clear chart", key="val_clear_chart"):
            st.session_state["val_rows"] = []
    with cB:
        if st.button("Start next half", key="val_next_half"):
            # Half-time resets (pistol + econ context), keep score
            cur_side = str(st.session_state.get("val_side", "Unknown"))
            # val_side is a selectbox storing value; flip if known
            if cur_side == "Attack":
            ...
            # Set default buy states for pistol/half start
    ...
    if len(st.session_state["val_rows"]) > 0:
        ...
        st.line_chart(plot_df.set_index("t"))
        st.dataframe(chart_df, use_container_width=True)
    else:
        st.info("Add at least one snapshot to see the chart.")
