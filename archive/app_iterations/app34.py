...
def estimate_inplay_prob(p0: float,
    ...
    """
    Simple in-play updater (MVP):
    - Starts at pre-match p0 (your fair odds model).
    - Nudges log-odds by score diff + econ diff + optional pistol info.
    - Adds: stage scaling (early score swings toned down), pistol decay (EMA-like leak),
      optional CT/T bias (map-aware), and a late-game "match point" nonlinearity.

    This is still a heuristic skeleton — but these tweaks reduce common live misbehavior
    (too jumpy early, too conservative late, pistol signal lingering forever).
    """
    ...
    # We implement this by re-centering the score_diff against the expected diff so far in the current half,
    # then adding a small "future side" bias that fades as the half ends.
    ...
    if side in ("CT", "T"):
        ...
        ct_adv = float(2.0 * ct_rate - 1.0)  # -1..+1, usually small positive on CT-sided maps

        # Expected score diff so far in this half from side alone:
        # If Team A is CT: expected positive drift; if T: expected negative drift.
    ...
    # Interprets the checkbox as: "Team A/B won the most recent pistol of the current half".
    if pistol_a is True or pistol_b is True:
        ...
        # Future side bias (small): being on CT on a CT-sided map slightly increases your chance to win *upcoming* rounds,
    # but this edge fades as the half runs out of rounds.
    if side in ("CT", "T"):
        ct_rate = CS2_CT_RATE_AVG
        if map_name:
            ct_rate = float(CS2_MAP_CT_RATE.get(str(map_name), CS2_CT_RATE_AVG))
    ...
    # Late-game nonlinearity: as a team nears the win target, probability should "snap" faster.
    lead_sign = 1 if score_diff > 0 else (-1 if score_diff < 0 else 0)
    if lead_sign != 0:
        leading_rounds = rounds_a if lead_sign > 0 else rounds_b
        # Ramp within last 3 rounds to victory (regulation target).
...
with tabs[3]:
    ...
    if "cs2_live_map_name" not in st.session_state:
        st.session_state["cs2_live_map_name"] = "Average (no map)"
    if "cs2_live_a_side" not in st.session_state:
...
# Faster economy input (buy state) — avoids typing exact $ totals during buy time.
    ...
    p_hat_map = estimate_inplay_prob(float(p0_map), int(rounds_a), int(rounds_b), float(econ_a), float(econ_b),
                                     ...
                                     map_name=None if str(map_sel).startswith("Average") else str(map_sel),
    ...
    with colAdd:
        if st.button("Add snapshot"):
            st.session_state["cs2_live_rows"].append({
                ...
                "econ_a": float(econ_a),
                "econ_b": float(econ_b),
                "map": None if str(map_sel).startswith("Average") else str(map_sel),
                "a_side": None if str(a_side_sel) == "Unknown" else str(a_side_sel),
                "pistol_a": bool(pistol_a),
                "pistol_b": bool(pistol_b),
