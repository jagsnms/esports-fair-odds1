...
with tabs[3]:
    ...
    # ---- Compute fair probability (map) + sigma (map) ----
    p_hat_map = estimate_inplay_prob(float(p0_map), int(rounds_a), int(rounds_b), float(econ_a), float(econ_b),
    ...
    sigma_map = estimate_sigma(float(p0_map), int(rounds_a), int(rounds_b), float(econ_a), float(econ_b), econ_missing=bool(econ_missing), chaos_boost=float(chaos_boost) + (0.02 if bool(econ_fragile) else 0.0))
    lo_map = max(0.01, p_hat_map - 2.0 * sigma_map)
    hi_map = min(0.99, p_hat_map + 2.0 * sigma_map)
...
with tabs[4]:
    ...
    sigma_map = estimate_sigma_valorant(
        p0=float(p0_map),
        rounds_a=int(val_rounds_a),
        rounds_b=int(val_rounds_b),
        eco_a_level=str(val_eco_a),
        ...
        missing_context_widen=float(missing_context_widen),
        total_rounds=24,
    )
    lo_map = max(0.01, p_hat_map - 2.0 * sigma_map)
    hi_map = min(0.99, p_hat_map + 2.0 * sigma_map)
