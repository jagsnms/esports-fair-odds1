...
with tabs[3]:
    ...
    with l2:
        ...
        cs2_inplay_persist_mode = st.selectbox("Persist mode", ["On score change","On any input change","Timed (throttled)"], index=1, key="cs2_inplay_persist_mode")
    with l3:
    ...
    st.session_state.setdefault("cs2_inplay_persist_every_s", float(st.session_state.get("cs2_inplay_persist_every_s", 2.0)))
    cs2_inplay_persist_every_s = st.number_input("Min seconds between logs (Timed mode)", min_value=0.5, max_value=30.0, step=0.5, format="%.1f", key="cs2_inplay_persist_every_s")
    ...
    colA, colB = st.columns(2)
    with colA:
        team_a_live = st.selectbox("Team A", df_cs2_live["team"].tolist(), key="cs2_live_a")
    with colB:
        team_b_live = st.selectbox("Team B", df_cs2_live["team"].tolist(), key="cs2_live_b")
    ...
    with c1:
        ...
        rounds_a = st.number_input("Rounds A", min_value=0, max_value=16, step=1, key="cs2_live_rounds_a")
    with c2:
        st.session_state.setdefault("cs2_live_rounds_b", int(st.session_state.get("cs2_live_rounds_b", 0)))
        rounds_b = st.number_input("Rounds B", min_value=0, max_value=16, step=1, key="cs2_live_rounds_b")
    ...
    kappa_map = float(kappa_map) * float(cs2_k_scale)
        ...
        if st.button("Add snapshot"):
            st.session_state["cs2_live_rows"].append({
                ...
                "sell_edge_pp": float((bid - p_hat)*100.0),
                        ...
                        # Optional: persist snapshots to CSV (change-aware so you can log full match progression)
            if bool(st.session_state.get("cs2_inplay_persist", False)) and str(st.session_state.get("cs2_inplay_match_id", "")).strip():
                try:
                    persist_mode = str(st.session_state.get("cs2_inplay_persist_mode", "On any input change"))
                    now_iso = datetime.now().isoformat(timespec="seconds")

                    # Build a signature so we only log when something meaningful changes.
                    sig_obj = {
                        "contract_scope": str(contract_scope),
                        "series_fmt": str(series_fmt),
                        "maps_a_won": int(maps_a_won),
                        "maps_b_won": int(maps_b_won),
                        "rounds_a": int(rounds_a),
                        "rounds_b": int(rounds_b),
                    ...
                    if persist_mode in ("On any input change", "Timed (throttled)"):
                        sig_obj.update({
                            "buy_state_a": str(buy_state_a),
                            "buy_state_b": str(buy_state_b),
                            "econ_missing": bool(econ_missing),
                            "econ_fragile": bool(econ_fragile),
                            "pistol_a": bool(pistol_a),
                            "pistol_b": bool(pistol_b),
                        ...
                        # If you have map/side inputs available (newer live logic), include them.
                        if "cs2_live_map" in st.session_state:
                            sig_obj["map"] = str(st.session_state.get("cs2_live_map", ""))
                        if "cs2_live_side_a" in st.session_state:
                            sig_obj["side_a_now"] = str(st.session_state.get("cs2_live_side_a", ""))

                    sig = json.dumps(sig_obj, sort_keys=True)
                    last_sig = st.session_state.get("cs2_inplay_last_sig")
                    last_ts = float(st.session_state.get("cs2_inplay_last_ts", 0.0))
                    now_ts = datetime.now().timestamp()

                    if persist_mode == "Timed (throttled)":
                        min_s = float(st.session_state.get("cs2_inplay_persist_every_s", 2.0))
                        do_persist = (now_ts - last_ts) >= min_s
                    else:
                        do_persist = (sig != last_sig)

                    if do_persist:
                        base_notes = str(st.session_state.get("cs2_inplay_notes", "")).strip()
                        auto_tag = f"[auto:{persist_mode}]"
                        notes = f"{base_notes} {auto_tag}".strip() if base_notes else auto_tag
                        ...
                        persist_inplay_row({
                            ...
                            "maps_a_won": int(maps_a_won),
                            "maps_b_won": int(maps_b_won),
                            "team_a": str(team_a_live),
                            "team_b": str(team_b_live),
                            "rounds_a": int(rounds_a),
                            "rounds_b": int(rounds_b),
                            "buy_state_a": str(buy_state_a),
                            "buy_state_b": str(buy_state_b),
                        ...
                        st.session_state["cs2_inplay_last_sig"] = sig
                        st.session_state["cs2_inplay_last_ts"] = now_ts
                except Exception as e:
...
with tabs[4]:
    ...
    with l2:
        ...
        val_inplay_persist_mode = st.selectbox("Persist mode", ["On score change","On any input change","Timed (throttled)"], index=1, key="val_inplay_persist_mode")
    with l3:
    ...
    st.session_state.setdefault("val_inplay_persist_every_s", float(st.session_state.get("val_inplay_persist_every_s", 2.0)))
    val_inplay_persist_every_s = st.number_input("Min seconds between logs (Timed mode)", min_value=0.5, max_value=30.0, step=0.5, format="%.1f", key="val_inplay_persist_every_s")
    show_inplay_log_paths()
    ...
    kappa_map = float(kappa_map) * float(val_k_scale)
    ...
    with colAdd:
        if st.button("Add snapshot", key="val_add_snap"):
            ...
            if bool(st.session_state.get("val_inplay_persist", False)) and str(st.session_state.get("val_inplay_match_id", "")).strip():
                try:
                    persist_mode = str(st.session_state.get("val_inplay_persist_mode", "On any input change"))
                    now_iso = datetime.now().isoformat(timespec="seconds")
                    ...
                    if persist_mode in ("On any input change", "Timed (throttled)"):
                        sig_obj.update({
                            "buy_state_a": str(val_buy_state_a),
                            "buy_state_b": str(val_buy_state_b),
                            "pistol_a": bool(val_pistol_a),
                            "pistol_b": bool(val_pistol_b),
                            "bid": float(val_bid),
                            "ask": float(val_ask),
                        })
                        if "val_side_a_now" in st.session_state:
                            sig_obj["side_a_now"] = str(st.session_state.get("val_side_a_now", ""))

                    sig = json.dumps(sig_obj, sort_keys=True)
                    last_sig = st.session_state.get("val_inplay_last_sig")
                    last_ts = float(st.session_state.get("val_inplay_last_ts", 0.0))
                    now_ts = datetime.now().timestamp()

                    if persist_mode == "Timed (throttled)":
                        min_s = float(st.session_state.get("val_inplay_persist_every_s", 2.0))
                        do_persist = (now_ts - last_ts) >= min_s
                    else:
                        do_persist = (sig != last_sig)

                    if do_persist:
                        base_notes = str(st.session_state.get("val_inplay_notes", "")).strip()
                        auto_tag = f"[auto:{persist_mode}]"
                        notes = f"{base_notes} {auto_tag}".strip() if base_notes else auto_tag
                        ...
                        persist_inplay_row({
                            ...
                            "pistol_a": bool(val_pistol_a),
                            "pistol_b": bool(val_pistol_b),
                            "gap_delta": int(val_stream.get("gap_delta", 0)) if 'val_stream' in locals() else 0,
                            "streak_winner": val_stream.get("streak_winner", "") if 'val_stream' in locals() else "",
                            "streak_len": int(val_stream.get("streak_len", 0)) if 'val_stream' in locals() else 0,
                            "reversal": bool(val_stream.get("reversal", False)) if 'val_stream' in locals() else False,
                            "n_tracked": int(val_stream.get("n_tracked", 0)) if 'val_stream' in locals() else 0,
                            "p0_map": float(val_p0_map),
                            "p_fair_map": float(val_p_hat_map),
                            "kappa_map": float(val_kappa_map),
                            "p_fair": float(val_p_hat),
                            "band_level": float(st.session_state.get("val_inplay_band_level", 0.80)),
                            "band_lo": float(val_lo),
                            "band_hi": float(val_hi),
                            "bid": float(val_bid),
                            "ask": float(val_ask),
                            "mid": float(val_mid),
                        ...
                        st.session_state["val_inplay_last_sig"] = sig
                        st.session_state["val_inplay_last_ts"] = now_ts
                except Exception as e:
                    st.warning(f"Snapshot not persisted: {e}")
