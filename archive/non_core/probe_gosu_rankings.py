...
def jiggle(page):
    # Kick every lazy loader and viewport sentinel we can
    try:
        for _ in range(3):
...
def main():
    ...
    ap.add_argument("--limit", type=int, default=3000, help="max teams to collect THIS RUN")
    ap.add_argument("--start-page", type=int, default=1, help="start crawling at this page number")
    ap.add_argument("--max-pages", type=int, default=1000, help="hard cap on how many pages to attempt")
    ...
    ap.add_argument("--out", type=str, default="", help="CSV path (stdout if omitted)")
    args = ap.parse_args()

    log(f"[start] headed={not args.headless} limit={args.limit} start={args.start_page} max_pages={args.max_pages} seed={bool(args.seed)} out={args.out or 'stdout'}")
    ...
    with sync_playwright() as p:
        ...
        if target > 1 and not click_page(page, target):
            ...
            steps = 0
            while get_current_page(page) < target and steps < (target + 50):
        ...
        while collected < args.limit and stalled < 3 and pages_visited < args.max_pages:
            ...
            for i in range(count):
                ...
                rows.append({"team": name, "team_id": team_id, "rank": rank, "rating": rating, "slug": slug})
                seen_slugs.add(slug)
    ...
    # Output (auto-append if file exists OR --append is set)
    fieldnames = ["team", "team_id", "rank", "rating", "slug"]
    if args.out:
        append_mode = args.append or os.path.exists(args.out)
        if append_mode:
            exists = os.path.exists(args.out)
            with open(args.out, "a", newline="", encoding="utf-8") as f:
                w = csv.DictWriter(f, fieldnames=fieldnames)
                if not exists or os.path.getsize(args.out) == 0:
                    w.writeheader()
                for r in rows:
                    w.writerow(r)
