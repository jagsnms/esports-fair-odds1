...
with tabs[3]:
    ...
    st.caption("Enter the current executable prices for Team A (bid = what you'd get selling YES, ask = what you'd pay buying YES).")
    ...
    # enforce ask >= bid for downstream calculations (don't mutate the widgets; just compute adjusted values)
    ...
    if len(st.session_state["live_rows"]) > 0:
        ...
        chart_df = chart_df[["t","p_hat","band_lo","band_hi","market_mid"]].set_index("t")
