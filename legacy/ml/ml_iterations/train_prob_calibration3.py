...
def infer_series_label(rows: pd.DataFrame) -> np.ndarray:
    ...
    if "team_a" in rows.columns and "team_b" in rows.columns:
        ta = rows["team_a"].astype(str).str.strip().str.lower()
        tb = rows["team_b"].astype(str).str.strip().str.lower()
        y = (w_str == ta).astype(float)
        neither = ~((w_str == ta) | (w_str == tb))
        y[neither] = np.nan
        return y.to_numpy()

    raise ValueError("Cannot infer winner label: missing team_a/team_b to match winner text.")
