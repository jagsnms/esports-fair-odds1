...
with tabs[3]:
    ...
    if contract_scope == "Map winner (this map)" or n_maps == 1:
        maps_a_won = 0
        maps_b_won = 0
    st.session_state["cs2_live_maps_a_won"] = int(maps_a_won)
    st.session_state["cs2_live_maps_b_won"] = int(maps_b_won)
    ...
    st.session_state["cs2_live_rounds_a"] = int(rounds_a)
    st.session_state["cs2_live_rounds_b"] = int(rounds_b)
    ...
    with colF2:
        ident = st.text_input("Ticker / token_id (or paste URL)", value=str(st.session_state.get("cs2_mkt_fetch_ident", "")), key="cs2_mkt_fetch_ident")
    with colF3:
