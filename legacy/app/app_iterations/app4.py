...
def load_dota_teams():
    df = pd.read_csv("dota2_full_team_list_with_slugs.csv")  # Must include: team, tier, rank, slug, opendota_id
...
def scrape_dota_matches_gosu(team_slug: str, team_name: str, target: int = 14, headed: bool = False):
    """
    Calls gosu_dota_scraper.py and returns a list of {"opponent": str, "win": bool}
    """
    try:
        cmd = [
            "python", "gosu_dota_scraper.py",
            "--team-slug", team_slug,
            "--team-name", team_name,
            "--target", str(target),
        ]
        if headed:  # keep False in Streamlit to avoid spawning a visible browser in prod
            cmd.append("--headed")

        result = subprocess.run(cmd, capture_output=True, text=True, timeout=180)
        if result.returncode != 0:
            st.error(f"Gosu scraper failed (exit {result.returncode}). Stderr:\n{result.stderr[:500]}")
            return []

        line = result.stdout.strip().splitlines()[-1]
        data = json.loads(line)

        # Normalize into your expected shape
        out = []
        for row in data:
...
# NEW OpenDota-based function (kept as an option)
def fetch_dota_matches(team_id, limit=15):
...
with tabs[1]:
    ...
    with col1:
        ...
        if st.button("Scrape Team A (Dota2)"):
            if src.startswith("OpenDota"):
                ...
                matches_a = scrape_dota_matches_gosu(team_a_slug, team_a, target=target_matches, headed=False)
    ...
    with col2:
        ...
        if st.button("Scrape Team B (Dota2)"):
            if src.startswith("OpenDota"):
                ...
                matches_b = scrape_dota_matches_gosu(team_b_slug, team_b, target=target_matches, headed=False)
    ...
    if st.button("Calculate (Dota2)"):
        if matches_a and matches_b:
            min_matches = min(len(matches_a), len(matches_b), target_matches)
            matches_a = matches_a[:min_matches]
            matches_b = matches_b[:min_matches]
            st.text(f"Using last {min_matches} matches for both teams.")
