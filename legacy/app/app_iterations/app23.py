...
# ==========================
# In-Play Logging (CS2 / Valorant)
# ==========================
INPLAY_LOG_PATH = APP_DIR / "inplay_kappa_logs.csv"
INPLAY_RESULTS_PATH = APP_DIR / "inplay_match_results.csv"
...
def fetch_kalshi_bid_ask(ticker: str):
    """
    Fetch best bid/ask for YES from Kalshi orderbook.

    Kalshi's orderbook endpoint returns bids for YES and bids for NO (no asks).
    In a binary market: YES ask at price P is equivalent to a NO bid at (100 - P).
    ...
    # Try a couple known base URLs; Kalshi has multiple API hosts depending on environment/category.
    bases = [
    ...
    for base in bases:
        try:
            ...
            # docs: keys are typically "yes" and "no" arrays of bid levels
            yes_bids = data.get("yes", []) or data.get("yes_bids", []) or data.get("yesOrders", [])
            no_bids  = data.get("no",  []) or data.get("no_bids",  []) or data.get("noOrders",  [])

            def _best_bid(levels):
                best_p = None
                best_sz = None
                for lvl in levels:
                    # common formats: {"price": 63, "quantity": 46} or {"price": "0.63", "size": "46"}
                    ...
                    try:
                        ...
                        continue
                    # If p looks like cents 0..100, keep. If 0..1, scale.
                    if p <= 1.0:
                        p_cents = p * 100.0
                    else:
                        p_cents = p
            ...
            # YES ask: 100 - best NO bid (since a NO bid at x is a YES ask at 100-x)
            ask = ((100.0 - best_no_c) / 100.0) if best_no_c is not None else None

            # If one side missing, we can't compute both; keep None and let UI handle
            meta = {
                "ticker": t,
                "base_url": base,
...
tabs = st.tabs(["CS2", "Dota2", "Diagnostics / Export", "CS2 In-Play Indicator (MVP)", "Valorant In-Play Indicator (MVP)"])
...
with tabs[2]:
    ...
    st.subheader("In-Play Calibration Status")
    ...
    def _safe_read_csv_cols(p: Path, cols: list[str]) -> Optional[pd.DataFrame]:
        if not p.exists():
            return None
        try:
            return pd.read_csv(p, usecols=cols, dtype=str)
        except Exception:
            try:
                return pd.read_csv(p, engine="python", on_bad_lines="skip", usecols=cols, dtype=str)
            except Exception:
                return None

    snap_rows = _count_csv_rows(INPLAY_LOG_PATH)
    res_rows = _count_csv_rows(INPLAY_RESULTS_PATH)

    logs_ids = _safe_read_csv_cols(INPLAY_LOG_PATH, ["timestamp","game","match_id","rounds_a","rounds_b","p_fair","kappa_map","bid","ask","spread_abs","gap_delta","streak_winner","streak_len","reversal","n_tracked"])
    res_ids = _safe_read_csv_cols(INPLAY_RESULTS_PATH, ["timestamp","game","match_id","winner"])

    snap_matches = int(logs_ids["match_id"].nunique()) if isinstance(logs_ids, pd.DataFrame) and "match_id" in logs_ids.columns else 0
    settled_matches = int(res_ids["match_id"].nunique()) if isinstance(res_ids, pd.DataFrame) and "match_id" in res_ids.columns else 0

    pending_matches = 0
    if isinstance(logs_ids, pd.DataFrame) and isinstance(res_ids, pd.DataFrame) and "match_id" in logs_ids.columns and "match_id" in res_ids.columns:
        pending_matches = int(len(set(logs_ids["match_id"].dropna().tolist()) - set(res_ids["match_id"].dropna().tolist())))

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Snapshots logged", snap_rows)
    c2.metric("Matches tracked", snap_matches)
    c3.metric("Matches settled", settled_matches)
    c4.metric("Pending settlement", pending_matches)

    with st.expander("In-play log files"):
        show_inplay_log_paths()

    with st.expander("Recent in-play rows"):
        if isinstance(logs_ids, pd.DataFrame) and not logs_ids.empty:
            st.dataframe(logs_ids.tail(12), use_container_width=True, height=260)
        else:
            st.info("No in-play snapshots found yet.")

        if isinstance(res_ids, pd.DataFrame) and not res_ids.empty:
            st.dataframe(res_ids.tail(12), use_container_width=True, height=200)
        else:
            st.info("No match settlements found yet.")

    with st.expander("Danger zone: reset in-play logs"):
        st.caption("Deletes in-play snapshot and settlement CSV files. This does not affect your main fair-odds log.")
        if st.button("Delete in-play log files", type="secondary"):
            try:
                if INPLAY_LOG_PATH.exists():
                    INPLAY_LOG_PATH.unlink()
                if INPLAY_RESULTS_PATH.exists():
                    INPLAY_RESULTS_PATH.unlink()
                st.success("Deleted in-play logs.")
                st.rerun()
            except Exception as e:
...
def estimate_sigma(p0: float,
                   ...
                   max_sigma: float = 0.25) -> float:
    """
    'Certainty band' width in probability-space.

    Practical intent (not academic):
      - Pre-match should NOT look wildly uncertain (you're just plotting your model prior).
      - Early game should allow wider bands (high variance / market overreaction).
      - Late game should tighten aggressively (information is mostly resolved).
      - Missing context (e.g., unknown buys) should widen, but it should not explode at t=0.

    Notes:
      - This is a heuristic uncertainty model. It is *not* "more points = Bayesian certainty"
        unless the points actually reduce the remaining degrees of freedom.
      - Bands are controlled elsewhere via the UI (sigma multiplier + sigma scale).
    """
    ...
    time_term = float(np.sqrt(frac_remaining))  # slow early, tight late

    # Pre-match confidence 0..1 (how far from coinflip)
    conf0 = min(1.0, abs(float(p0) - 0.5) * 2.0)

    # If the match hasn't started, don't penalize for missing econ — there is no econ yet.
    if rp <= 0:
        sigma0 = base * (0.85 - 0.20 * conf0)  # ~0.068 at p0=0.50, tighter for big favorites
        return float(min(max_sigma, max(min_sigma, sigma0)))

    # "Lock" term: if leader is close to winning, tighten bands quickly.
    lead_score = max(ra, rb)
    rounds_to_win = max(0, 13 - lead_score)  # 0..13
    lock_term = 0.30 + 0.70 * (rounds_to_win / 13.0)  # near win => ~0.30, early => ~1.0

    # Early-game uncertainty hump (wide after pistol/first guns, then decays).
    # Peaks around ~4-8 rounds, then fades.
    early_term = 0.08 * (rp / 4.0) * float(np.exp(-rp / 6.0))

    # Economy uncertainty: only apply if we have a usable econ proxy (not missing/unknown).
    econ_unc = 0.0
    if (not bool(econ_missing)) and float(econ_a) > 0.0 and float(econ_b) > 0.0:
        ...
        econ_unc = 0.06 * (1.0 - econ_stability) * (0.60 + 0.40 * time_term)

    # Missing context widening: ramp in AFTER the match starts (avoid huge pre-match bands).
    missing_unc = 0.0
    if bool(econ_missing):
        ramp = min(1.0, rp / 4.0)  # 0->1 over first 4 rounds
        missing_unc = 0.04 * ramp * (0.60 + 0.40 * time_term)

    # Base component: anchored to your pre-match model quality, shrinks with time + lock.
    sigma_core = base * (0.65 + 0.35 * time_term) * (1.0 - 0.25 * conf0) * lock_term

    # Mild "more observations" tightening (caps quickly; avoids fake certainty).
    n_eff = min(rp, 12)
    snapshot_factor = 1.0 / float(np.sqrt(1.0 + (n_eff / 6.0)))  # ~1.0 -> ~0.577

    sigma = (sigma_core + early_term + econ_unc + missing_unc + float(chaos_boost)) * snapshot_factor

    return float(min(max_sigma, max(min_sigma, sigma)))

# --------------------------
# NEW — Confidence bands v2 (Kappa + Beta credible intervals) + streak tracking
# --------------------------
...
with tabs[3]:
    ...
    st.caption("Tip: Use 'Add snapshot' as you update the score. The app infers streaks when you update in order; if you skip rounds, it treats that as a gap and widens bands.")
    match_id_cs2 = st.text_input("Match ID (for logging/settlement)", value=str(st.session_state.get("match_id_cs2","")), key="match_id_cs2")
    persist_cs2 = st.checkbox("Persist snapshots to CSV (inplay_kappa_logs.csv)", value=bool(st.session_state.get("persist_cs2", False)), key="persist_cs2")
    if persist_cs2:
        show_inplay_log_paths()
    ...
    with cctx1:
        series_fmt = st.selectbox("Series format", ["BO1", "BO3", "BO5"], index=int(st.session_state.get("live_series_fmt_idx", 1)))
    with cctx2:
        contract_scope = st.selectbox("Contract priced on", ["Map winner (this map)", "Series winner"], index=int(st.session_state.get("live_contract_scope_idx", 0)))
    with cctx3:
        maps_a_won = st.number_input("Maps A won", 0, 4, int(st.session_state.get("live_maps_a_won", 0)), 1)
    with cctx4:
        maps_b_won = st.number_input("Maps B won", 0, 4, int(st.session_state.get("live_maps_b_won", 0)), 1)

    st.session_state["live_series_fmt_idx"] = ["BO1","BO3","BO5"].index(series_fmt)
    st.session_state["live_contract_scope_idx"] = ["Map winner (this map)","Series winner"].index(contract_scope)
    ...
    if contract_scope == "Map winner (this map)" or n_maps == 1:
        maps_a_won = 0
        maps_b_won = 0
    st.session_state["live_maps_a_won"] = int(maps_a_won)
    st.session_state["live_maps_b_won"] = int(maps_b_won)
    ...
    if contract_scope == "Series winner" and n_maps > 1:
        ...
        p0_series = st.number_input("Pre-match fair series win% for Team A (0–1)", min_value=0.01, max_value=0.99,
                                    value=float(st.session_state.get("live_p0_series", 0.55)),
                                    step=0.01, format="%.2f")
        st.session_state["live_p0_series"] = float(p0_series)
        ...
        p0_map = st.number_input("Pre-match fair win% for Team A (0–1)", min_value=0.01, max_value=0.99,
                                 value=float(st.session_state.get("live_p0_map", 0.60)),
                                 step=0.01, format="%.2f")
        st.session_state["live_p0_map"] = float(p0_map)
    ...
    if "live_buy_a" not in st.session_state:
        st.session_state["live_buy_a"] = "Skip/Unknown"
    if "live_buy_b" not in st.session_state:
        st.session_state["live_buy_b"] = "Skip/Unknown"

    with c3:
        buy_a = st.selectbox("Team A buy state", BUY_STATES, key="live_buy_a")
    with c4:
        buy_b = st.selectbox("Team B buy state", BUY_STATES, key="live_buy_b")
    ...
    st.session_state["live_rounds_a"] = int(rounds_a)
    st.session_state["live_rounds_b"] = int(rounds_b)

    # ---- Round flow auto-tracking (streaks / gaps) ----
    cs2_stream = update_round_stream(prefix="cs2", rounds_a=int(rounds_a), rounds_b=int(rounds_b))
    sc1, sc2, sc3 = st.columns(3)
    with sc1:
        if cs2_stream.get("streak_winner") is None:
            st.metric("Current streak", "—")
        else:
            st.metric("Current streak", f'{cs2_stream.get("streak_winner")} x{cs2_stream.get("streak_len")}')
    with sc2:
        gd = int(cs2_stream.get("gap_delta", 0) or 0)
        if gd > 1:
            st.metric("Last update", f"gap +{gd-1} rounds")
        else:
            st.metric("Last update", "ok")
    with sc3:
        st.metric("Reversal", "Yes" if bool(cs2_stream.get("reversal")) else "No")
    # Keep numeric econ in session_state for downstream calcs / exports.
    st.session_state["live_econ_a"] = int(econ_a)
    st.session_state["live_econ_b"] = int(econ_b)
    ...
    with colP1:
        pistol_a = st.checkbox("A won most recent pistol?", value=bool(st.session_state.get("live_pistol_a", False)), key="live_pistol_a")
    with colP2:
        pistol_b = st.checkbox("B won most recent pistol?", value=bool(st.session_state.get("live_pistol_b", False)), key="live_pistol_b")
    with colChaos:
        chaos_boost = st.slider("Chaos widen (manual)", 0.00, 0.25, float(st.session_state.get("live_chaos", 0.00)), 0.01)
    st.session_state["live_chaos"] = float(chaos_boost)
    ...
    colF1, colF2, colF3 = st.columns([1.1, 2.2, 1.0])
    with colF1:
        venue = st.selectbox("Venue", ["Manual", "Kalshi", "Polymarket"], index=0, key="mkt_fetch_venue")
    with colF2:
        ident = st.text_input("Ticker / token_id (or paste URL)", value=str(st.session_state.get("mkt_fetch_ident", "")), key="mkt_fetch_ident")
    with colF3:
        do_fetch = st.button("Fetch bid/ask", use_container_width=True)

    if do_fetch and venue != "Manual":
        try:
            if venue == "Kalshi":
                bid_f, ask_f, meta = fetch_kalshi_bid_ask(ident)
            else:
                bid_f, ask_f, meta = fetch_polymarket_bid_ask(ident)

            if bid_f is not None:
                st.session_state["live_market_bid"] = float(bid_f)
            if ask_f is not None:
                st.session_state["live_market_ask"] = float(ask_f)

            st.session_state["mkt_fetch_meta"] = meta
            st.success(f"Fetched {venue} bid/ask.")
        except Exception as e:
            st.error(f"Fetch failed: {e}")

    meta = st.session_state.get("mkt_fetch_meta")
    if meta:
        with st.expander("Last fetch details (info)"):
            st.json(meta)

    colBid, colAsk = st.columns(2)
    with colBid:
        market_bid = st.number_input("Best bid (Sell) for Team A (0–1)", min_value=0.01, max_value=0.99,
                                     value=float(st.session_state.get("live_market_bid", 0.48)),
                                     step=0.01, format="%.2f")
    with colAsk:
        market_ask = st.number_input("Best ask (Buy) for Team A (0–1)", min_value=0.01, max_value=0.99,
                                     value=float(st.session_state.get("live_market_ask", 0.52)),
                                     step=0.01, format="%.2f")
    ...
    st.session_state["live_market_bid"] = float(market_bid)
    st.session_state["live_market_ask"] = float(market_ask)
    ...
    # ---- Compute fair probability (map) ----
    p_hat_map = estimate_inplay_prob(p0=float(p0_map), rounds_a=int(rounds_a), rounds_b=int(rounds_b),
                                     eco_a=int(econ_a), eco_b=int(econ_b),
    ...
    # ---- Confidence bands v2: Beta credible interval driven by K (certainty) ----
    st.markdown("### Certainty band controls (v2)")
    bc1, bc2 = st.columns([1.2, 1.0])
    with bc1:
        cs2_ci_level = st.select_slider(
            "Band confidence (credible interval)",
            options=[0.68, 0.75, 0.80, 0.90, 0.95],
            value=float(st.session_state.get("cs2_ci_level", 0.80)),
            key="cs2_ci_level",
        )
    with bc2:
        cs2_kappa_scale = st.slider(
            "K scale (tighten/loosen)",
            min_value=0.50,
            max_value=2.00,
            value=float(st.session_state.get("cs2_kappa_scale", 1.00)),
            step=0.05,
            key="cs2_kappa_scale",
        )
    ...
    kappa_map = compute_kappa_cs2(
        ...
        streak_len=int(cs2_stream.get("streak_len", 0)),
        streak_winner=cs2_stream.get("streak_winner"),
        reversal=bool(cs2_stream.get("reversal", False)),
        gap_delta=int(cs2_stream.get("gap_delta", 0)),
        chaos_boost=float(chaos_boost),
        total_rounds=24,
    ) * float(cs2_kappa_scale)

    lo_map, hi_map = beta_credible_interval(float(p_hat_map), float(kappa_map), float(cs2_ci_level))
    ...
    with colM2:
        st.metric(f"Certainty band ({int(cs2_ci_level*100)}% CI)", f"[{lo*100:.1f}%, {hi*100:.1f}%]")
    with colM3:
    ...
    with st.expander("Show underlying MAP fair (debug)"):
        st.write({
            ...
            "ci_level": float(cs2_ci_level),
            "kappa_map": float(kappa_map),
            "cs2_stream": cs2_stream,
    ...
    with colAdd:
        if st.button("Add snapshot"):
            ...
            # Optional: persist the latest snapshot to disk for later calibration
            if persist_cs2 and match_id_cs2:
                try:
                    entry = {
                        "timestamp": datetime.now().isoformat(timespec="seconds"),
                        "game": "cs2",
                        "match_id": match_id_cs2,
                        "contract_scope": contract_scope,
                        ...
                        "rounds_a": int(rounds_a),
                        "rounds_b": int(rounds_b),
                        "buy_state_a": str(buy_state_a) if "buy_state_a" in locals() else "",
                        "buy_state_b": str(buy_state_b) if "buy_state_b" in locals() else "",
                        "econ_missing": bool(econ_missing),
                        "econ_fragile": bool(econ_fragile),
                        "pistol_a": bool(pistol_a),
                        "pistol_b": bool(pistol_b),
                        ...
                        "streak_len": int(cs2_stream.get("streak_len", 0)),
                        "reversal": bool(cs2_stream.get("reversal", False)),
                        "n_tracked": int(cs2_stream.get("n_tracked", 0)),
                        "p0_map": float(p0_map),
                        "p_fair_map": float(p_hat_map),
                        "kappa_map": float(kappa_map),
                        "p_fair": float(p_hat),
                        "band_level": float(cs2_ci_level),
                        "band_lo": float(lo),
                        "band_hi": float(hi),
                        "bid": float(bid),
                        "ask": float(ask),
                    ...
                    persist_inplay_row(entry)
                except Exception as e:
                    st.warning(f"In-play persist failed: {e}")
    ...
    with st.expander("Settle match (writes winner for calibration)"):
        if not match_id_cs2:
            st.info("Enter a Match ID above to settle and tie snapshots to an outcome.")
        else:
            winner = st.selectbox("Winner (for this Match ID)", options=["A","B"], key="cs2_winner_select")
            if st.button("Write settlement", key="cs2_write_settlement"):
                try:
                    persist_inplay_result(match_id_cs2, "cs2", team_a, team_b, winner)
                    st.success("Saved.")
                except Exception as e:
                    st.error(f"Could not save settlement: {e}")

    st.markdown("### Chart")
    if len(st.session_state["live_rows"]) > 0:
        chart_df = pd.DataFrame(st.session_state["live_rows"])

        # plot fair + band + market for whichever scope you're trading
        plot_df = chart_df[["t","p_hat","band_lo","band_hi","market_mid"]].set_index("t")
...
with tabs[4]:
    ...
    st.caption("Tip: As long as you update scores in order, the app infers streaks automatically. If you skip several rounds, it logs a gap and widens bands (honest uncertainty).")
    match_id_val = st.text_input("Match ID (for logging/settlement)", value=str(st.session_state.get("match_id_val","")), key="match_id_val")
    persist_val = st.checkbox("Persist snapshots to CSV (inplay_kappa_logs.csv)", value=bool(st.session_state.get("persist_val", False)), key="persist_val")
    if persist_val:
        show_inplay_log_paths()
    ...
    # ---- Round flow auto-tracking (streaks / gaps) ----
    val_stream = update_round_stream(prefix="val", rounds_a=int(val_rounds_a), rounds_b=int(val_rounds_b))
    vc1, vc2, vc3 = st.columns(3)
    with vc1:
        if val_stream.get("streak_winner") is None:
            st.metric("Current streak", "—")
        else:
            st.metric("Current streak", f'{val_stream.get("streak_winner")} x{val_stream.get("streak_len")}')
    with vc2:
        gd = int(val_stream.get("gap_delta", 0) or 0)
        if gd > 1:
            st.metric("Last update", f"gap +{gd-1} rounds")
        else:
            st.metric("Last update", "ok")
    with vc3:
        st.metric("Reversal", "Yes" if bool(val_stream.get("reversal")) else "No")
    ...
    with colF2:
        val_ident = st.text_input("Ticker / token_id (or paste URL)", value=str(st.session_state.get("val_mkt_fetch_ident", "")), key="val_mkt_fetch_ident")
    with colF3:
    ...
    vmeta = st.session_state.get("val_mkt_fetch_meta")
    if vmeta:
        with st.expander("Last fetch details (info)"):
            st.json(vmeta)
    ...
    bc1, bc2 = st.columns([1.2, 1.0])
    with bc1:
        val_ci_level = st.select_slider(
            "Band confidence (credible interval)",
            options=[0.68, 0.75, 0.80, 0.90, 0.95],
            value=float(st.session_state.get("val_ci_level", 0.80)),
            key="val_ci_level",
        )
    with bc2:
        val_kappa_scale = st.slider(
            "K scale (tighten/loosen)",
            min_value=0.50,
            max_value=2.00,
            value=float(st.session_state.get("val_kappa_scale", 1.00)),
            step=0.05,
            key="val_kappa_scale",
        )
    ...
    kappa_map = compute_kappa_valorant(
        ...
        pistol_a=bool(val_pistol_a),
        pistol_b=bool(val_pistol_b),
        streak_len=int(val_stream.get("streak_len", 0)),
        streak_winner=val_stream.get("streak_winner"),
        reversal=bool(val_stream.get("reversal", False)),
        gap_delta=int(val_stream.get("gap_delta", 0)),
        chaos_boost=float(chaos_boost),
        total_rounds=24,
    ) * float(val_kappa_scale)

    lo_map, hi_map = beta_credible_interval(float(p_hat_map), float(kappa_map), float(val_ci_level))
    ...
    with m1:
        st.metric(line_label, f"{p_hat*100:.1f}%")
    with m2:
        st.metric(f"Certainty band ({int(val_ci_level*100)}% CI)", f"[{lo*100:.1f}%, {hi*100:.1f}%]")
    with m3:
        st.metric("Market bid / ask", f"{bid*100:.1f}% / {ask*100:.1f}%")
    with m4:
    ...
    with colAdd:
        if st.button("Add snapshot", key="val_add_snap"):
            ...
            # Optional: persist the latest snapshot to disk for later calibration
            if persist_val and match_id_val:
                try:
                    entry = {
                        "timestamp": datetime.now().isoformat(timespec="seconds"),
                        "game": "valorant",
                        ...
                        "econ_missing": False,
                        "econ_fragile": False,
                        "pistol_a": bool(val_pistol_a) if "val_pistol_a" in locals() else False,
                        "pistol_b": bool(val_pistol_b) if "val_pistol_b" in locals() else False,
                        "gap_delta": int(val_stream.get("gap_delta", 0)),
                        "streak_winner": val_stream.get("streak_winner"),
                        "streak_len": int(val_stream.get("streak_len", 0)),
                        "reversal": bool(val_stream.get("reversal", False)),
                        "n_tracked": int(val_stream.get("n_tracked", 0)),
                        "p0_map": float(p0_map),
                        "p_fair_map": float(p_hat_map),
                        "kappa_map": float(kappa_map),
                        "p_fair": float(p_hat),
                        "band_level": float(val_ci_level) if "val_ci_level" in locals() else float(cs2_ci_level),
                        "band_lo": float(lo),
                        "band_hi": float(hi),
                        "bid": float(bid),
                        "ask": float(ask),
    ...
    with colClear:
        if st.button("Clear snapshots", key="val_clear_snaps"):
            st.session_state["val_rows"] = []
    with colExport:
        if st.button("Export snapshots to Diagnostics", key="val_export_snaps"):
            st.session_state["live_export_df"] = pd.DataFrame(st.session_state["val_rows"])
    ...
    with st.expander("Settle match (writes winner for calibration)"):
        if not match_id_val:
            ...
            winner = st.selectbox("Winner (for this Match ID)", options=["A","B"], key="val_winner_select")
            if st.button("Write settlement", key="val_write_settlement"):
                try:
                    persist_inplay_result(match_id_val, "valorant", val_team_a, val_team_b, winner)
                    st.success("Saved.")
                except Exception as e:
                    st.error(f"Could not save settlement: {e}")
