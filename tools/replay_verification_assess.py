#!/usr/bin/env python3
"""
Stage 4/4B: Bounded replay/verification assessment for PHAT initiative.
...
Stage 4B: Pre-loads payloads in script and injects into runner so assessment
can process real replay input (unblocks runner lazy-load path resolution in script context).
No compute or calibration changes.
"""
...
def _load_replay_payloads(replay_path_str: str) -> list[dict[str, Any]]:
    """Load payloads the same way the runner would: BO3 entries first, then generic JSONL. (Stage 4B: script-side load to unblock.)"""
...
async def run_assessment(replay_path: str) -> dict[str, Any]:
    ...
    # Stage 4B: Pre-load and inject payloads so the runner processes them (unblocks lazy-load path resolution in script context).
    runner._replay_payloads = payloads
    runner._replay_index = 0
    runner._replay_path = replay_path_str
    runner._replay_match_id = None
    runner._replay_format = "raw" if (payloads and _is_raw_bo3_snapshot(payloads[0])) else ("point" if payloads else None)
...
def main() -> None:
    replay_path = sys.argv[1] if len(sys.argv) > 1 else "logs/bo3_pulls.jsonl"
