#!/usr/bin/env python3
"""
Stage 4: Bounded replay/verification assessment for PHAT initiative.
...
No compute or calibration changes. Output: JSON summary for assessment report.
"""
...
async def run_assessment(replay_path: str) -> dict[str, Any]:
    ...
    direct_load_count = len(load_generic_jsonl(replay_path_str))
    ...
    broadcaster = type("B", (), {"broadcast": lambda self, msg: None})()
    ...
    for i in range(max_ticks):
        if getattr(runner, "_replay_payloads", None) is not None:
    ...
    payload_count = len(getattr(runner, "_replay_payloads", [])) or 0
    return {
        "replay_path": replay_path_str,
        "replay_path_exists": path.exists(),
        "direct_load_payload_count": direct_load_count,
        "replay_payload_count_loaded": payload_count,
