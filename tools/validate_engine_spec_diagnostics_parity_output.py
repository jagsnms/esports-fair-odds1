#!/usr/bin/env python3
"""
Validate one parity-output artifact against engine_spec_diagnostics_parity schema.
"""
...
SCHEMA_VERSION = "engine_spec_diagnostics_parity_output_validator.v1"
ROOT = Path(__file__).resolve().parent.parent
DEFAULT_SCHEMA_PATH = ROOT / "tools" / "schemas" / "engine_spec_diagnostics_parity.schema.json"
...
def _extract_artifact_metadata(artifact_obj: Any) -> dict[str, Any]:
    out = {
        "schema_version": None,
        "status": None,
        "parity_mode": None,
...
def validate_engine_spec_diagnostics_parity_output(
    *,
    artifact_path: Path,
    schema_path: Path,
    ...
    if str(artifact_path).strip() == "":
        result["errors"] = ["invalid artifact_path: required explicit non-empty path"]
        return 1, result
    ...
    if artifact_err is not None:
        ...
        return 1, result
    result["artifact_metadata"] = _extract_artifact_metadata(artifact_obj)
...
def main() -> int:
    ...
    exit_code, payload = validate_engine_spec_diagnostics_parity_output(
        artifact_path=Path(args.artifact_path or ""),
        schema_path=Path(args.schema_path or ""),
