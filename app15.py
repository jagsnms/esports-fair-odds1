...
def estimate_sigma(p0: float,
    ...
    sigma = (base * time_term * (1.0 - 0.30 * conf0)) * lock_term + econ_unc + float(chaos_boost)
...
with tabs[3]:
    ...
    sigma_map = estimate_sigma(float(p0_map), int(rounds_a), int(rounds_b), float(econ_a), float(econ_b), float(chaos_boost))
