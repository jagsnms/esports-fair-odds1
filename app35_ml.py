...
def _ensure_csv(path: Path, columns: list[str]):
    """Ensure CSV exists and has the requested schema (adds missing columns if needed)."""
    if not path.exists():
        pd.DataFrame([], columns=columns).to_csv(path, index=False)
        return

    # If schema changed, backfill missing columns with blanks and rewrite header once.
    try:
        existing_cols = list(pd.read_csv(path, nrows=0).columns)
    except Exception:
        # If the file is corrupt/unreadable, don't overwrite it silently.
        raise
    ...
    if missing or existing_cols != columns:
        df = pd.read_csv(path)
        for c in missing:
            df[c] = ""
        # drop extras? keep extras at end but enforce canonical order first
        # Keep any unknown/legacy columns after the canonical ones.
...
def persist_inplay_map_result(match_id: str, game: str, map_index: int, map_name: str,
                             team_a: str, team_b: str, winner: str):
    """Persist a single map result (for map-level ML)."""
