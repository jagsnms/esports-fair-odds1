...
**All Cursor Cloud agent work MUST be committed to the branch `cursor/agent-work`.** At the start of every session, check out this branch (create it from `agent-base` if it doesn't exist yet). This keeps all agent-produced changes isolated from the user's own branches so they can be compared side-by-side.

```bash
# At session start:
git fetch origin
git checkout cursor/agent-work 2>/dev/null || git checkout -b cursor/agent-work origin/agent-base
git pull origin cursor/agent-work --rebase 2>/dev/null || true
```

Do not commit to `master`, `agent-base`, or any other user branch.
